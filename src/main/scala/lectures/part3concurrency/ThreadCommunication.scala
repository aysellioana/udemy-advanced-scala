...
object ThreadCommunication extends App{
  ...
  def smartProdCons(): Unit = {
    val contaier = new SimpleContainer
    val consumer = new Thread(() => {
      println("[consumer] waiting...")
      contaier.synchronized{
        contaier.wait()
      }
      //container must have some value
      println("[consumer] I have consumer " + contaier.get)
    })
    val producer = new Thread(() => {
  ...
  //smartProdCons()
    ...
    def prodConsLArgeBuffer(): Unit = {
      ...
      val consumer = new Thread(() => {
        ...
        while(true){
          buffer.synchronized{
            if(buffer.isEmpty){
              println("[consumer] buffer wmpty, waiting..." )
              buffer.wait()
            }
   ...
   class Consumer(id: Int, buffer: mutable.Queue[Int]) extends Thread {
     override def run(): Unit = {
       ...
       while (true) {
         buffer.synchronized {
           /*
           producer produce a value, two Cons are waiting
           notifies ONE consumer, notifies on buffer
           notifies the other consumer

            */
           while (buffer.isEmpty) {
             println(s"[consumer $id] buffer wmpty, waiting...")
             buffer.wait()
           }
           //there must be at least ONe value in the buffer
           val x = buffer.dequeue()
   ...
   def testNotifyAll(): Unit = {
     ...
     new Thread(() => {
       Thread.sleep(2000)
       println("[announcer] Rock'n rolll")
       bell.synchronized{
         bell.notifyAll()
       }
     }).start()
   ...
   case class Friend(name: String){
     def bow(other: Friend) = {
       this.synchronized{
         ...
         other.rise(this)
         println(s"$this: my firned $other has risen")
       }
     }
