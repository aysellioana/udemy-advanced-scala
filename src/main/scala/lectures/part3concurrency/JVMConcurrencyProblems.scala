...
object JVMConcurrencyProblems {
  ...
  def buySafe(bankAccount: BankAccount, thing: String, price: Int): Unit = {
    bankAccount.synchronized { //does not allow multiple threads to run the cristical section AT THE SAME TIME
  ...
  def demoBankingProblem(): Unit = {
    (1 to 10000).foreach { _ =>
      ...
      thread1.join()
      thread2.join()
      if(account.amount != 43000) println(s"AHA! I'v just broken the bank: ${account.amount}")
    }
  }
  ...
  min value = 1
    ...
    all threads ( in parallel) comute 0 + 1 = 1
  ...
  main thread:
    message = "Scala sucks"
    awesomeThread.start()
    sleep(1001) = yields executution
  awsome thread:
    sleep(1000) - yields execution
  OS  gives the CPU to some import thread, takes > 2s
